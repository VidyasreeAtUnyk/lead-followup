...
// -- Agent loop, shared by both providers (src/agent/loop.ts, loopGemini.ts) --
// Turn cap, retry count, backoff, and the worthwhile-retry-after cutoff are
// provider-agnostic loop control, not OpenAI-specific -- same values, one
// definition, used by whichever provider is actually running.
...
// -- Provider-specific (genuinely different per provider, not shared) --
...
// Gemini Flash-tier pricing differs from OpenAI's, so this is deliberately a
// separate constant, not a shared one -- same "compare runs relatively" caveat.
export const ESTIMATED_COST_PER_TOKEN_GEMINI = 0.0000002;

/** Gemini model used for every agent turn; override via GEMINI_MODEL env var. */
