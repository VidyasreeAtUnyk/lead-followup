...
export const retryTests: Test[] = [
  ...
  },
  {
    ...
    run: async () => {
                        ...
                        tool_calls: [
                          {
                            id: "call_1",
                            type: "function",
                            function: { name: "escalate_to_agent", arguments: JSON.stringify({ lead_id: 6, reason: "test" }) },
                          },
                        ],
                      },
                    },
      ...
      const result = await runAgentForLead(db, 6, stubClient);
  ...
  },
  {
    ...
    run: async () => {
      ...
      const result = await runAgentForLead(db, 7, stubClient, { maxRetries: 1, baseDelayMs: 1000 });
